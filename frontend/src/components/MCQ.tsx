...
const MCQ = ({
  ...
  const [hasSubmitted, setHasSubmitted] = useState(!!initialSelectedOption);

  const handleOptionSelect = (optionId: string) => {
    if (hasSubmitted) return;

    if (allow_multiple) {
      // Future: Handle multiple selection
  ...
  const isCorrect = hasSubmitted && selectedOptionData?.isCorrect;
          ...
          >
            ...
            <div className="text-sm text-muted-foreground">
              {isCorrect ? (
                ...
                <span>
                  You selected "
                  <Markdown allowHtml={false} latex={true} className="inline">
